...
u32 switch_value;
short switch_7_flag = 0;
short switch_6_flag = 0;
short switch_5_flag = 0;
short switch_4_flag = 0;
short stop_timer = 0;
short count = 0;
u16 debounce = 0;
...
unsigned int screenshot_pointer[307200];
unsigned int old_frame[307200];
...
void switch_interrupt_handler()
{
	...
	if(new_switch_value != switch_value)
	{
		...
		if(switch_value & SW7 && !switch_7_flag)
		{
			int j = 0;
			for(j = 0; j < 10; j++)
			{
				screenshot_pointer[j] = 2;
				xil_printf("%d: 0x%x\n", j+1, screenshot_pointer[j]);
			}
			takeScreenShot(framePointer, screenshot_pointer);
			for(j = 0; j < 10; j++)
			{
				xil_printf("%d: 0x%x\n", j+1, screenshot_pointer[j]);
			}
		...
		else if(!(switch_value & SW7) && switch_7_flag)
			switch_7_flag = 0;

		if(switch_value & SW5 && !switch_5_flag)
		{
			takeScreenShot(framePointer, old_frame);
			takeScreenShot(screenshot_pointer, framePointer);
			//framePointer = screenshot_pointer;
			switch_5_flag = 1;
			stop_timer = 1;
			xil_printf("6");
		}
		else if(!(switch_value & SW5) && switch_5_flag)
		{
			takeScreenShot(old_frame, framePointer);
			redraw();
			stop_timer = 2;
			count = 5;
			switch_5_flag = 0;
		...
		if(switch_value & SW6 && !switch_6_flag)
		{
			takeScreenShotSoftware(framePointer, screenshot_pointer);
			switch_6_flag = 1;
			xil_printf("6");
		}
		else if(!(switch_value & SW6) && switch_6_flag)
			switch_6_flag = 0;

		if(switch_value & SW4 && !switch_4_flag)
		{
			switch_4_flag = 1;
			xil_printf("4");
		}
		else if(!(switch_value & SW4) && switch_4_flag)
			switch_4_flag = 0;
	}
