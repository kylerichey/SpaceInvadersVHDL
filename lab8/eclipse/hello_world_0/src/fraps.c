...
void takeScreenShot(unsigned int *framePointer, unsigned int *screenshot_pointer)
{
	...
	FRAPS_mWriteReg(XPAR_FRAPS_0_BASEADDR,FRAPS_SLV_REG2_OFFSET, 307199);
...
void takeScreenShotSoftware(unsigned int *framePointer, unsigned int *screenshot_pointer)
{
	int i = 0;
	for(i = 0; i < 307200; i++)
	{
		screenshot_pointer[i] = framePointer[i];
	}
}
